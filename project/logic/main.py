...
def calculate_rcpm_with_local_sgs(
    cur, df_operations, df_resources, selected_tasks, use_pr=False
):
    operations = prepare_operations(df_operations)
...
Your choice: """)
        ...
        if act == "1":
            ...
            table_name = input(
                "Enter the table('operations', 'resources', 'additional_info', 'current_status', 'results'): "
            )
            ...
            table = input(
                "Enter table name for manual input ('operations', 'resources', 'additional_info', 'current_status'): "
            )
...
Your choice: """)
        ...
        if act == "1":
            ...
            calculate_rcpm_with_local_sgs(
                cur, df_operations, df_resources, selected_tasks
            )
        else:
            print("Такого действия нет!")
...
Your choice: """)
        ...
        if act == "1":
            ...
            completed_tasks = df_current_status[df_current_status["is_done"] == True][
                "op_id"
            ].tolist()
            df_operations_filtered = df_operations[
                ~df_operations["op_id"].isin(completed_tasks)
            ]
